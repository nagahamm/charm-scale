...
const CHAT_SCHEMA = {
  ...
  properties: {
    ...
    summary: { type: "string" },
    metrics: { type: "array", minItems: 4, maxItems: 6, items: METRIC },
    timeline: {
      type: "array",
      items: {
        type: "object",
...
- metrics は「返信速度」「文量バランス」「質問返し」「感情表現」「主導権」など、根拠のある観点を4〜6個。
...
- metrics は「第一印象」「表情」「構図・画質」「服装・清潔感」「背景・シチュエーション」など4〜6個。
...
export default async (req) => {
  ...
  const mode = payload?.mode === "photo" ? "photo" : "chat";
  ...
  blocks.push({
    ...
    text:
      mode === "chat"
        ? `上記${images.length}枚のスクリーンショットを時系列順の会話として読み、スキーマに従って分析結果を返して。${
            context ? `\n\n補足情報(相談者による申告):\n${context}` : ""
          }`
        : `上記${images.length}枚のプロフィール写真を評価し、スキーマに従って結果を返して。${
            context ? `\n\n補足情報(相談者による申告):\n${context}` : ""
          }`,
  });
    ...
    async start(controller) {
      ...
      try {
        const run = client.messages.stream({
          model: MODEL,
          max_tokens: 32000,
          system: mode === "chat" ? CHAT_SYSTEM : PHOTO_SYSTEM,
          thinking: { type: "adaptive" },
          output_config: {
            effort: "medium",
            format: { type: "json_schema", schema: mode === "chat" ? CHAT_SCHEMA : PHOTO_SCHEMA },
          },
